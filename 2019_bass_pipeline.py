...
class Pipeline:
    def __init__(self, folder_name):
        ...
        for filename in glob.glob(folder_name + '/*.csv'):
            try:
                df1 = pd.read_csv(filename, names=['key', 'value'], skiprows=3, nrows=14)
                df_temp = pd.read_csv(filename, skiprows=17, skipinitialspace=True)
                try:
                ...
                except IndexError:  # If there is no 3rd dataframe in csv
                    df2 = df_temp
                    df3 = None
    ...
    def calc_cum_data(self, empty_value):
        ...
        for df_group in self.df_list:
            ...
            for behav, dur in zip(df_group[1]['BehavCode'], df_group[1]['Duration']):
                ...
                # changing behaviors to standard values
                if behav == 'Tactile/expl':
                    behav = 'Tactile/explore'
                elif behav == 'Threat' or behav == 'Display':
                    behav = 'Threat/display'
                elif behav == 'Mantle shake':
                    behav = 'Mantleshake'
                elif behav == 'Self direct' or behav == 'Self-direct':
                    behav = 'Selfdirect'
                elif behav == 'Hair pluck':
                    behav = 'Groom'
                elif behav == 'Fear grimace' or behav == 'Fear Grimace':
                    behav = 'Feargrimace'
                elif behav == 'Self bite':
                    behav = 'Selfbite'
                elif behav == 'Head toss':
                    behav = 'Headtoss'
                elif behav == 'Salute/eye poke' or behav == 'Social':
                    behav = 'Salute'
                elif behav == 'Sex: self other':
                    behav = 'Sex/self/other'
                elif behav == 'Drink':
                    behav = 'Eat/drink'
...
if __name__ == '__main__':
    ...
    try:
        ...
        print('Argument Error: 2019_bass_pipeline.py only takes 2 arguements!')
